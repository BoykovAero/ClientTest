...
> **Нет желания возиться с терминалом?** Есть путь без своего компьютера —
> [CLOUD.md](CLOUD.md): сервисный аккаунт Google вместо OAuth и хостинг,
> собирающий контейнер прямо из GitHub. Терминал там не нужен ни разу.
...
> **Python.** Нужен 3.9 или новее; на Ubuntu 24.04 штатный 3.12 подходит.
> Проверить: `python3 -V`. Если на Mac отдаёт 3.9 — этого хватит для
> `setup_env.py` и `google_auth_setup.py`, но для разработки удобнее поставить
> свежий с python.org или через `brew install python@3.12`.
...
| `Conflict: can't use getUpdates method while webhook is active` | На токене висит вебхук: Telegram отдаёт апдейты или в него, или в поллинг, но не в оба. Бот снимает вебхук сам при каждом старте — если строка вернулась, вебхук ставят заново, а значит токен знает кто-то ещё: `/revoke` у @BotFather и новый токен. Посмотреть, чей вебхук: `https://api.telegram.org/bot<ТОКЕН>/getWebhookInfo` |
